...
(* malo bolj esteticno
let rec intbool_list f_int f_bool ib_list =
  let rec mapper = function
  | Nil -> Nil
  | Int (i, ibl) -> Int (f_int i, mapper ibl)
  | Bool (b, ibl) -> Bool (f_bool i, mapper ibl)
  in mapper *)
...
let rec intbool_separate ib_list = 
  let rec ib_sep acc_i acc_b = function
    | Nil -> (acc_i, acc_b)
    | Int (i, ibl) -> ib_sep (i :: acc_i) acc_b ibl
    | Bool (b, ibl) -> ib_sep acc_i (b :: acc_b) ibl
  in
  ib_sep [] [] ib_list
...
let rec update counter = function
  ...
  | Frost -> {counter with frost = counter.fire + 1}
